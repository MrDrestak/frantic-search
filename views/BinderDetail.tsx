...
const BinderDetail: React.FC<BinderDetailProps> = ({ binderId, onBack }) => {
  ...
  const handleAddCard = async () => {
    ...
    try {
        ...
        if (cards.length >= check.limit) {
            alert(`Limit Reached. Your current plan allows ${check.limit} unique cards in this binder.`);
  ...
  const handleWhatsAppShare = () => {
      ...
      let specificPhrase = binder.type === BinderType.WISHLIST ? "Estoy buscando estas cartas, en estas ediciones:" : "Tengo este binder que puede interesar:";
    ...
    <div className="p-4 md:p-8 space-y-6 pb-24 h-[calc(100vh-4rem)] flex flex-col relative">
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-xl max-w-sm w-full p-6 shadow-2xl animate-in fade-in zoom-in-95">
                <div className="flex flex-col items-center text-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">Are you sure you want to delete <span className="text-white font-medium">"{binder.name}"</span>? This action cannot be undone and will remove all {cards.length} cards in it.</p>
                    </div>
                    <div className="flex gap-3 w-full mt-2">
                        <button onClick={() => setShowDeleteModal(false)} className="flex-1 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg transition-colors font-medium">Cancel</button>
                        <button onClick={handleConfirmDeleteBinder} className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors font-medium">Delete</button>
                    </div>
                </div>
            </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
              <div className="bg-slate-900 border border-slate-700 rounded-xl max-w-sm w-full p-6 shadow-2xl animate-in fade-in zoom-in-95">
                  <div className="flex justify-between items-start mb-4">
                      ...
                      <button onClick={() => setEditingPriceCard(null)} className="text-slate-400 hover:text-white"><X size={20} /></button>
                  </div>
                  <p className="text-sm text-slate-400 mb-4">Set the price you want for <span className="text-white font-medium">{editingPriceCard.name}</span>.</p>
                  <div className="space-y-4">
                      <div>
                          <label className="block text-xs uppercase font-bold text-slate-500 mb-1">Currency</label>
                          <div className="grid grid-cols-2 gap-2">
                              <button type="button" onClick={() => setCurrencyInput('USD')} className={`py-2 px-3 rounded border text-sm font-medium ${currencyInput === 'USD' ? 'bg-violet-600 border-violet-500 text-white' : 'bg-slate-950 border-slate-700 text-slate-400'}`}>USD ($)</button>
                              <button type="button" onClick={() => setCurrencyInput('PEN')} className={`py-2 px-3 rounded border text-sm font-medium ${currencyInput === 'PEN' ? 'bg-violet-600 border-violet-500 text-white' : 'bg-slate-950 border-slate-700 text-slate-400'}`}>SOL (S/)</button>
                          </div>
                      </div>
                      <div>
                          <label className="block text-xs uppercase font-bold text-slate-500 mb-1">Price</label>
                          <div className="relative"><div className="absolute left-3 top-2.5 text-slate-400 font-bold">{currencyInput === 'USD' ? '$' : 'S/'}</div><input type="number" step="0.01" min="0" value={priceInput} onChange={(e) => setPriceInput(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-white focus:ring-2 focus:ring-violet-500 outline-none text-lg font-mono" placeholder="0.00" autoFocus /></div>
                      </div>
                      <button onClick={handleSavePrice} className="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 mt-2"><DollarSign size={18} /> Save Offer</button>
                  </div>
              </div>
          </div>
      )}
                ...
                {searchStep === 'CONFIG' && selectedCard && (
                     <div className="flex-1 overflow-y-auto min-h-0 pb-10">
                        <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 md:p-6 flex flex-col md:flex-row gap-6 md:gap-8 animate-in fade-in zoom-in-95 duration-200 mt-2 md:mt-10">
                            <div className="flex-shrink-0 mx-auto md:mx-0"><img src={getCardImage(selectedCard)} alt={selectedCard.name} className="w-48 md:w-64 rounded-xl shadow-2xl border border-slate-800" /></div>
                            <div className="flex-1 space-y-6 md:space-y-8">
                                <div><h3 className="text-2xl md:text-3xl font-bold text-white">{selectedCard.name}</h3><div className="flex items-center gap-3 mt-2"><span className="text-lg md:text-xl text-violet-400">{selectedCard.set_name}</span><span className="text-sm bg-slate-800 px-2 py-1 rounded text-slate-400 font-mono border border-slate-700">#{selectedCard.collector_number}</span></div></div>
                                ...
                                {binder.type !== BinderType.AUCTION && (
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="flex items-center bg-slate-950 border border-slate-700 rounded-lg overflow-hidden h-11">
                                                <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-11 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-900 transition-colors"><Minus size={18}/></button>
                                                <input type="number" min="1" value={quantity} onChange={e => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="w-16 bg-transparent text-center text-white font-bold outline-none border-x border-slate-700 h-full" />
                                                <button onClick={() => setQuantity(q => q + 1)} className="w-11 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-900 transition-colors"><Plus size={18}/></button>
                                            </div>
                                            <span className="text-xs text-slate-500">Copies of this specific version</span>
                                        </div>
                                    </div>
                                )}
                                ...
                                {binder.type === BinderType.AUCTION && (
                                    <div className="space-y-4 bg-amber-500/5 border border-amber-500/20 p-4 rounded-xl"><h4 className="text-amber-400 font-bold flex items-center gap-2"><Gavel size={18} /> Auction Configuration</h4><div className="bg-blue-900/30 border border-blue-800 rounded p-3 text-xs text-blue-200 flex items-start gap-2"><Clock size={16} className="mt-0.5 shrink-0" /><p>All auctions end at <strong>22:00 GMT-5</strong> (Lima/Bogota/Quito) on the selected date. Each auction is for exactly <b>1</b> copy.</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-xs uppercase font-bold text-slate-500 mb-1">Finish Date</label><div className="relative"><Calendar className="absolute left-3 top-2.5 text-slate-500" size={16} /><input type="date" value={auctionEndDate} onChange={(e) => setAuctionEndDate(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-10 pr-3 py-2 text-white focus:ring-2 focus:ring-amber-500 outline-none" /></div></div><div><label className="block text-xs uppercase font-bold text-slate-500 mb-1">Currency</label><select value={auctionCurrency} onChange={(e) => setAuctionCurrency(e.target.value as any)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-amber-500 outline-none"><option value="USD">USD ($)</option><option value="PEN">SOL (S/)</option></select></div><div><label className="block text-xs uppercase font-bold text-slate-500 mb-1">Base Price</label><input type="number" step="0.01" value={basePrice} onChange={(e) => setBasePrice(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-amber-500 outline-none" /></div><div><label className="block text-xs uppercase font-bold text-slate-500 mb-1">Direct Buy Price</label><input type="number" step="0.01" value={buyItNowPrice} onChange={(e) => setBuyItNowPrice(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-amber-500 outline-none" /></div></div></div>
                                )}

                                <div className="flex gap-3 pt-4"><button onClick={handleAddCard} className="flex-1 bg-violet-600 hover:bg-violet-500 text-white py-3 md:py-4 rounded-xl font-bold flex justify-center items-center gap-2 shadow-xl shadow-violet-900/20 text-lg transition-all hover:scale-[1.02]"><Check size={24} /> {binder.type === BinderType.AUCTION ? "Create Auction" : "Add to Binder"}</button></div>
                            </div>
                        </div>
                     </div>
                )}
      ...
      <div className="flex-1 overflow-y-auto pr-2 pb-20">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            {cards.length === 0 && (<div className="col-span-full text-center py-20 text-slate-500"><Layers className="w-16 h-16 mx-auto mb-4 opacity-20" /><p>This binder is empty.</p>{isOwner && <button onClick={() => setShowSearch(true)} className="text-violet-400 underline mt-2">Start adding cards</button>}</div>)}
            {cards.length > 0 && filteredCards.length === 0 && (<div className="col-span-full text-center py-12 text-slate-500"><p>No cards match "{filterText}"</p><button onClick={() => setFilterText('')} className="text-violet-400 hover:text-violet-300 mt-2 text-sm font-medium">Clear Filter</button></div>)}
        </div>
      </div>
    </div>
  );
