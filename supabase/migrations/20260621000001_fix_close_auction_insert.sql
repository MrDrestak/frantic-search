-- Fix close_auction and close_expired_auctions:
--   1. Include seller_name (NOT NULL column that was missing)
--   2. Remove card_id / binder_id (columns that don't exist in trade_interactions)
...
BEGIN
  ...
  IF rec.top_bidder_id IS NOT NULL THEN
    ...
    INSERT INTO public.trade_interactions
      (seller_id, buyer_id, seller_name, buyer_name, card_name, status)
    VALUES
      (rec.user_id, rec.top_bidder_id,
       COALESCE(seller_name, 'Vendedor'),
